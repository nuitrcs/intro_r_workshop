...
<!--
To Knit the file without toc for printing
rmarkdown::render('basics_and_datatypes.Rmd',
                  output_options=list(toc=FALSE,toc_float=FALSE),
                  output_file = 'basics_and_datatypes_notes.html',
...
Note that `load` will overwrite any objects already in your workspace that have the same names as objects in the file you are loading.  I don't recommend using `save` and `load`.  It's generally better to save specific data files instead.
...
```{r}
install.packages(c("psych", "DT", "data.table"))
```

...
`data.table` package also has functions for reading in data.  The `fread` function is currently the fastest implementation of reading a rectangular standardized data file into R.

R also has packages for reading other structured files like XML and JSON, or interfacing with databases.  For more of using R with databases, see the R section of the [Databases workshop materials](https://github.com/nuitrcs/databases_workshop).  
